...
@section('breadcrumbs')
    {{ Breadcrumbs::render('dashboard_pembayaran_edit', $pembayaran) }}
@endsection
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('pembayaran.update', ['pembayaran' => $pembayaran]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            ...
                            <select id="select_kk" name="kk" data-placeholder="Pilih Kartu Keluarga" class="custom-select w-100 @error('kk') is-invalid @enderror">
                                <option value="{{ $pembayaran->kk->id }}" selected>{{ $pembayaran->kk->no_kk }} - {{ $pembayaran->kk->nama_kepala_keluarga }}</option>
                            </select>
                            @error('kk')
                         ...
                         <div class="form-group">
                            ...
                            <select id="jenis_pembayaran" name="jenis_pembayaran" class="form-control @error('jenis_pembayaran') is-invalid @enderror">
                                ...
                                <option value="PBB" {{ $pembayaran->jenis_pembayaran == 'PBB' ? 'selected' : '' }}>PBB</option>
                                <option value="Iuran" {{ $pembayaran->jenis_pembayaran == 'Iuran' ? 'selected' : '' }}>Iuran</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input id="total_pembayaran" value="{{ old('total_pembayaran', $pembayaran->total_pembayaran) }}" name="total_pembayaran" type="text" class="form-control @error('total_pembayaran') is-invalid @enderror" placeholder="Input Total Pembayaran" />
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('keterangan') is-invalid @enderror" name="keterangan" placeholder="Input Keterangan">{{old('keterangan', $pembayaran->keterangan)}}</textarea>
                            <!-- error message -->
                            @error('keterangan')
                              <span class="invalid-feedback">
